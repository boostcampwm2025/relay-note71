# [week3.md](http://week3.md/) - team 71

> J050_김아연,J198_이신욱,J147_송해찬,J128_박현정
> 
...
- 새로 생성할 퀘스트에 대한 우리 팀의 논의
    ...
    - 슬랙 커뮤니티에 적극적인 사람들과 그렇지 않은 사람들로 나뉨
        
        → **적극지 않던 사람들도 적극적으로 참여할 수 있게**
        
    - 달성 기준을 주 1회 보다는 좀 더 높여서 (최소 격일) 퀘스트 참여 빈도를 높이자
        
        → **한 두번 해보기 시작하면 소통의 즐거움을 느끼고**,
        
        → **슬랙에 질문이나 글을 올리는 것에 거부감을 줄이고 더 많이 올리게 된다.**
        
    - 슬랙에 글을 올릴 때, 릴레이 프로젝트 언급 금지하자
        
        → **상단에 반복적인 릴레이프로젝트 언급이 슬랙 커뮤니티를 피로하게 만들 수 있다.**
        

---

> 주의: 퀘스트 수행 시 릴레이 프로젝트 언급 금지!! ❌
> 
...
```
부스트캠프 슬랙 커뮤니티를 활성화하기 위한 목적으로,
캠퍼들이 서로에 대해 가볍게 이야기 나눌 수 있는 재미있고 짧은 TMI 질문을 생성해줘.
...
> 
- **배경** :
    - AI가 그려준 그림에 대해 실제 이미지인지 아닌지 판단하는 퀘스트를 슬랙에서 본 적이 있는데,
    - 그걸 응용해서 ‘이번엔 내가 그린 그림을 맞춰보라고 하면 어떨까?’ 하는 생각에서 아이디어가 나오게 되었다.
- **프롬포트** :
...
- 선택한 퀘스트: **"🐴 개발 밸런스 게임 – 당신의 선택은?"**

- 이유:
    - 개발 과정에서 분석, 설계, 구현까지 각각의 개발자가 추구하는 스타일, 방식, 관념이 다르기 때문
    - 서로 다른 개발 철학과 접근법을 공유하며, 토론하는 과정이 의미있는 활동이 될 것 같음 토론하는 괒
    - 누구나 한번 쯤 고민해봤거나 관심 있는 주제라서, 공감대와 흥미를 이끌어내기 좋아 커뮤니티 활성화에 기여할 수 있을 것 같음
...
2. 5개의 질문 중 하나를 선정 (답변 예시)

```
개발자 밸런스 게임 질문 5가지

코드 리뷰: 꼼꼼하게 한 줄씩 VS 큰 그림 위주로 빠르게
커밋 메시지: 상세하게 작성 VS 간결하게 핵심만 작성
협업 소통: 실시간 채팅(슬랙, 카카오톡) 선호 VS 이슈 트래커(지라, 깃허브 이슈) 선호
개발 환경: IDE에 많은 플러그인 설치해 최적화 VS 최소한의 기본 기능만 사용
테스트 작성: TDD(테스트 주도 개발) 철저히 지키기 VS 기능 개발 후 나중에 테스트 작성
```

3. `random` 채널에 공유
4. 토론 내용에 대한 느낀점을 **Gist (학습정리.md)**에 작성

<br/>

### J203_이유경

- 선택한 퀘스트: 2. "내가 직접 그린 그림 AI에게 맞춰보라고 하기"
- 이유
    - 이미지에 대한 AI의 추측 능력이 어느 정도인지 파악해보기 위함
- 수행 계획
    1. 다음 프롬프트 + 직접 그린 그림을 GPT에 입력
    ...
    2. 결과 확인
    3. 주 1회 수행 하고 slack에 내용 공유
 
<br/>

### J252_조아령

- 선택한 퀘스트: “AI가 주는 일일 챌린지”
- 이유:
    - AI가 어떤 미션을 만들어 낼 지 궁금함
    - 팀원들과 소통하며 편한 피어 피드백 분위기를 만들고 싶음
- 수행 계획
    - 여러 AI를 돌려가며 미션을 받아본다
    - 최대한 여러 명의 동료들에게 보낼 수 있도록 노력한다

<br/>

### S021_오명진

- 선택한 퀘스트: **🐰 퀘스트 3. "AI가 주는 일일 챌린지"**
- 이유: 랜덤으로 무슨 미션을 줄 지 궁금하고, 팀원들과 관계도 좋아질 것 같음!
- 수행 계획:
    - 4주차 매일매일 AI가 제공해주는 미션을 수행해보기!
